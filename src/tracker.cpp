...
int url_info_parse(adx_url_info_t *url_info, char *remoteaddr, char *requestparam)
{
...
int adx_tracker(adx_context_t *context_info, adx_url_info_t *url_info, adx_dsp_info_t *dsp_info, char *remoteaddr, char *requestparam)
{
    int errcode = E_SUCCESS;
    /********************************/
    /* URL 参数错误检测 */
    /********************************/
    url_info_parse(url_info, remoteaddr, requestparam);
	...
	// 发送kafka
	adx_tracker_kafka_send(context_info, url_info, dsp_info);
	adx_log(LOGINFO, "[%c][%d][%s][0x%X][adx_tracker][KAFKA]", url_info->mtype, url_info->adx, url_info->bid.c_str(), errcode);
    }
...
int adx_dsp_doit(FCGX_Request *request, adx_pool_t *pool, adx_context_t *context_info)
{
	...
	errcode = adx_tracker(context_info, url_info, dsp_info, remoteaddr, requestparam);
    }

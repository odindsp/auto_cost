...
int adx_dsp_control_count(adx_context_t *context_info, adx_url_info_t *url_info, adx_dsp_info_t *dsp_info, ADX_DSP_COUNTER_COUNT_T *dsp_count)
{
	...
	errcode = adx_redis_cluster_command(conn, "HINCRBY  dsp10minutecounter_%lu_%d imp_%d_%lu_%d_%s 1", // 自动出价
		policyid, TIME_HOUR_10M(),
	...
	errcode = adx_redis_cluster_command(conn, "HINCRBY  dsp10minutecounter_%lu_%d clk_%d_%lu_%d_%s 1", // 自动出价
		policyid, TIME_HOUR_10M(),
		url_info->at, mapid, url_info->adx, url_info->gp.c_str());
    }
	...
	errcode = adx_redis_cluster_command(conn, dsp_count->p_cost_10m,"HINCRBY dsp10minutecounter_%lu_%d cost_%d_%d_%lu_%s %d", // 10分钟计数
		policyid, TIME_HOUR_10M(), // key
    ...
    // 10分钟计数销毁时间
    errcode = adx_redis_cluster_command(conn, "EXPIRE dsp10minutecounter_%lu_%d %d", policyid, TIME_HOUR_10M(), get_time_sec_30min_end());
    if (errcode != E_SUCCESS) {
	// TODO: 错误处理
    }
